...
impl SVF {
    ...
    pub fn run_svf_nonlinear(&mut self, input: f32) -> f32 {
        ...
        if est_source_a0 != 0. {
            ...
            a[0] = (v_t * (est_source_a0/i_s).sinh()) / est_source_a0;
            // a[0] = ((est_source_a0).tanh()) / est_source_a0;
        }
        let est_source_rest = [
            (input - a[0] * est_source_a0 - self.get_estimate(1, est_type, input)),
            self.get_estimate(0, est_type, input),
        ];
        for n in 0..est_source_rest.len() {
            if est_source_rest[n] != 0. {
                a[n + 1] = est_source_rest[n].tanh() / est_source_rest[n];
            }
        }
        // ---------- calculations ----------
        // factored out of the equation
        let g1 = 1. / (1. + g * a[1] * a[0] * k);
        let g2 = 1. / (1. + g1 * g.powi(2) * a[1] * a[2]);

        // solving equations for output voltages at v1 and v2
        self.vout[0] = (g * a[1] * (input - self.s[1]) + self.s[0]) * g1 * g2;
        self.vout[1] = g * a[2] * self.vout[0] + self.s[1];
...
impl PluginParameters for FilterParameters {
    ...
    fn get_parameter_text(&self, index: i32) -> String {
        match index {
            ...
            1 => format!("{:.3}", 1. / self.res.get()),
            2 => format!("{:.3}", self.drive.get()),
